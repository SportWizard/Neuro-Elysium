...
2. Extract the files to `PathOfGame` and run the game
3. Go to `PathOfGame\BepInEx\interop` and copy the following to `.\NeuroElysium\lib`
   - `Assembly-CSharp.dll`
   - `DialogueSystem.dll`
   - `Il2Cppmscorlib.dll`
   - `Il2CppSystem.dll`
...
4. Run `dotnet build` in `.\NeuroElysium`
5. Copy `.\NeuroElysium\bin\Debug\net6.0\NeuroElysium.dll` to `PathOfGame\BepInEx\plugins`
